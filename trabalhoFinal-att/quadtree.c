...
unsigned char calculate_average(unsigned char *pData, int total_width, int start_x, int start_y, int size) {
    long sum = 0;
...
int is_homogeneous_tolerant(unsigned char *pData, int total_width, int start_x, int start_y, int size, int threshold) {
    ...
    for (int j = 0; j < size; j++) {
        for (int i = 0; i < size; i++) {
            ...
            if (abs(current_pixel - avg_value) > threshold) {
                return 0;
            }
        }
    }
...
int is_homogeneous(unsigned char *pData, int total_width, int start_x, int start_y, int size, int threshold) {
...
QuadNode* build_quadtree(unsigned char *pData, int total_width, int start_x, int start_y, int size, int threshold) {
    ...
    if (size == 1 || is_homogeneous_tolerant(pData, total_width, start_x, start_y, size, threshold)) {
        ...
        if (size > 1) printf("FOLHA ENCONTRADA: Regiao Homogenea em (%d, %d) com tamanho %d. Valor: %hhu\n", start_x, start_y, size, node->value);
        return node;
    } printf("RECURSAO: Regiao heterogenea em (%d, %d) com tamanho %d.\n", start_x, start_y, size);

    node->is_leaf = 0; 
    int half_size = size / 2;

    node->children[0] = build_quadtree(pData, total_width, start_x, start_y, half_size, threshold);
    node->children[1] = build_quadtree(pData, total_width, start_x + half_size, start_y, half_size,threshold);
    node->children[2] = build_quadtree(pData, total_width, start_x, start_y + half_size, half_size, threshold);
    node->children[3] = build_quadtree(pData, total_width, start_x + half_size, start_y + half_size, half_size, threshold);
